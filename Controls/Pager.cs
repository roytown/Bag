...
namespace Controls
{
    ...
    public class Pager : WebControl, IPostBackEventHandler, INamingContainer
    {
        ...
        /// </summary>
        private string _SHOWING_RESULT = "当前条数";
        [Category("Globalization")]
        ...
        private string RenderGoToScript()
        {
            ...
            sb.AppendFormat("gotoImg.src = gotoElem.style.display == 'none' ? '{0}' : '{1}';",
                                    Page.ClientScript.GetWebResourceUrl(this.GetType(), "Controls.Images.arr_right.gif"),
                                    Page.ClientScript.GetWebResourceUrl(this.GetType(), "Controls.Images.arr_left.gif")
                                    );
            sb.Append("}");
